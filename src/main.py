...
>>>>>>> Stashed changes
    ...
    def func(population: Population, total: int) -> Tuple[Individual]:
        return tuple(
            generate_individual(population)
            for _ in range(transform_number_of_nodes(population.amount, total))
        )

    base_nodes: Tuple[Individual] = tuple(
        flatten((func(e, total_nodes) for e in populations))
    )
    complement_nodes: Tuple[Individual] = tuple(
        generate_individual(population)
        for population in islice(
            cycle(default_populations), total_nodes - len(base_nodes)
        )
    )
...
def connect_node(
    node: Individual, nodes: List[Individual], populations: Tuple[Population]
):
    population = next(el for el in populations if el.status == node.status)
    [first, second] = [
        population.connection_number.first,
        population.connection_number.second,
    ]
    number_of_connections = randint(first, second)
    valid_connects = [
        sub_node
        for sub_node in nodes
        if sub_node.id not in sub_node.connections
        and node.id not in sub_node.connections
    ]
    candidate_connections = [
        sub_node
        for sub_node in valid_connects
        if not sub_node.id == node.id and sub_node.id not in node.connections
    ]
    ...
    connections: List[str] = [
        ...
        for el in np.random.choice(
            candidate_connections, size=number_of_connections, replace=False
        )
    ]
...
def generate_connections(
    nodes: Tuple[Individual], populations: Tuple[Population]
) -> Tuple[Individual]:
...
def populate_nodes(graph: Graph, node_list):
    g = graph.copy()
    for el in node_list:
...
def populate_edges(graph: Graph, individual_list: Tuple[Individual]):
    g = graph.copy()
    ...
    connection_pairs = [
        (individual.name, get_individual_from_uid(connection).name)
...
def color(status, populations):
    return next(
        filter(lambda population: population.status == status, populations)
    ).color
